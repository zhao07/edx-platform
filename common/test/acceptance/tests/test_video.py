...
class YouTubeVideoTest(VideoBaseTest):
    ...
    @skip("Not sure why this one is failing")
...
class Html5VideoTest(VideoBaseTest):
    ...
    def test_video_component_fully_rendered_in_html5_mode(self):
        ...
        Then all sources are correct
        """
        self.navigate_to_video()

        # Verify that the video has rendered in "HTML5" mode
        self.assertTrue(self.video.is_video_rendered('html5'))

        # Verify that the page has video source urls that match exactly with `HTML5_SOURCES`
        self.assertItemsEqual(self.video.all_video_sources, HTML5_SOURCES)
