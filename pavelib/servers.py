...
def run_server(system, settings=None, port=None, fast=False):
    """
    ...
    `fast` if set, will start the server without compiling/collecting assets.
    """
    ...
    if not fast:
        call_task('pavelib.assets.update_assets', args=[system, '--settings={}'.format(settings)])
...
def lms(options):
    ...
    run_server('lms', settings=settings, port=port, fast=fast)
...
def studio(options):
    ...
    port = getattr(options, 'port', None)
    fast = getattr(options, 'fast', False)
    run_server('studio', settings=settings, port=port, fast=fast)
...
def devstack(args):
    ...
    run_server(args.system[0], settings='devstack', fast=args.fast)
...
def run_all_servers(options):
    ...
    worker_settings = getattr(options, 'settings', 'dev_with_worker')

    run_multi_processes([
        django_cmd('lms', settings, 'runserver', '--traceback', '--pythonpath=.', DEFAULT_PORT['lms']),
        django_cmd('studio', settings, 'runserver', '--traceback', '--pythonpath=.', DEFAULT_PORT['studio']),
